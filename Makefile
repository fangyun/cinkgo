...
# Uncomment one of the options below to change the way Cinkgo is built.
...
# Compile Cinkgo with external plugin support ?
...
OBJS = $(EXTRA_OBJS) util.o player_builder.o coord_system.o board.o player.o gtp.o stone.o point.o comparable.o debug.o cinkgo.o 
...
test: FORCE
	...
	@ ./cinkgo -d0 -t =1000 < gtp/genmove.gtp  2>cinkgo.log >/dev/null
	@echo "OK"

	@echo -n "Testing quiet mode...    "
	@if  grep -q '.' < cinkgo.log ; then \
		echo "FAILED:";  cat cinkgo.log;  exit 1;  else  echo "OK"; \
	fi
...
distribute: FORCE
	...
	cp cinkgo distribute/

        ifndef MSYS2
		cd distribute  &&  strip cinkgo
        else
		cd distribute  &&  strip cinkgo.exe
		@echo "packing exe ..."
		@cd distribute  &&  upx -o p.exe cinkgo.exe  &&  mv p.exe cinkgo.exe
                ifndef MSYS2_STATIC
			@echo "copying dlls ..."
			@cd distribute; \
...
install: distribute
	$(INSTALL) -d $(BINDIR)
	$(INSTALL) distribute/cinkgo $(BINDIR)/
...
clean:: clean-recursive
	-@rm cinkgo build.h >/dev/null 2>&1
	@echo ""
